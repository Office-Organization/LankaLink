...
class _FemaleHeadedSummaryScreenState extends State<FemaleHeadedSummaryScreen> {
  ...
  // දත්ත ගබඩා කිරීමට සහ Filter කිරීමට අවශ්‍ය List
  List<Map<String, dynamic>> _allRecords = [];
  List<Map<String, dynamic>> _filteredRecords = [];
  
  // GN Division Dropdown එක සඳහා
  List<String> _gnDivisions = ['සියලුම වසම්'];
  String _selectedGN = 'සියලුම වසම්';
    ...
    try {
        ...
        if (members.isNotEmpty) {
          ...
          if (headMember['gender'] == 'ස්ත්‍රී') {
            ...
            extracted.add({
              ...
              'phone': phone,
              'income': income,
              'gn': gnDivision, // GN Division එක Save කරගැනීම
      ...
      if (mounted) {
        setState(() {
          ...
          _gnDivisions.addAll(uniqueGNs.toList()..sort()); // GN ලැයිස්තුව අකාරාදී පිළිවෙලට සැකසීම
          _isLoading = false;
        });
      }
    } catch (e) {
  ...
  // දත්ත Filter කිරීමේ ක්‍රියාවලිය
  void _applyFilters() {
    List<Map<String, dynamic>> temp = _allRecords;

    // 1. GN Division එකෙන් Filter කිරීම
    if (_selectedGN != 'සියලුම වසම්') {
      temp = temp.where((record) => record['gn'] == _selectedGN).toList();
    }

    // 2. Search Box එකෙන් Filter කිරීම (නම හෝ ගෘහ මූලික අංකය)
  ...
  // 2. PDF එක Generate කිරීමේ ක්‍රියාවලිය
  Future<void> _generatePDF() async {
    ...
    try {
      ...
      await Printing.sharePdf(
        bytes: pdfBytes,
        filename: 'Female_Headed_Families${_selectedGN != "සියලුම වසම්" ? "_$_selectedGN" : ""}.pdf',
      );
      ...
      body: Stack(
        children: [
                      ...
                      Container(
                        ...
                        child: DropdownButtonHideUnderline(
                          child: DropdownButton<String>(
                            ...
                            items: _gnDivisions.map((String gn) {
                ...
                child: _isLoading
                    ...
                    : _filteredRecords.isEmpty
                        ...
                        : ListView.builder(
                            ...
                            itemCount: _filteredRecords.length + 1, // +1 for the count header
                                ...
                                child: Padding(
                                  ...
                                  child: Column(
                                    ...
                                    children: [
                                      ...
                                      _buildSummaryRow(Icons.map_outlined, 'GN වසම:', record['gn']),
          ...
          if (_filteredRecords.isNotEmpty)
            Positioned(
              left: -99999, // මෙය තිරයෙන් පිටත (invisible) render වේ
              child: RepaintBoundary(
                key: _pdfTableKey,
                child: Container(
                  width: 1100, // A4 Landscape පළල
                  color: Colors.white,
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      Text(
                        'කාන්තා මූලික පවුල්${_selectedGN != "සියලුම වසම්" ? " - $_selectedGN වසම" : ""}',
