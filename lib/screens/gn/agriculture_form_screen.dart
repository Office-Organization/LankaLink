...
class _AgricultureFormViewState extends State<_AgricultureFormView> {
  ...
  bool _isFormEmpty(AgricultureViewModel vm) {
    return _locationNameCtrl.text.trim().isEmpty &&
        ...
        vm.selectedDevelopmentCategory == null &&
        vm.selectedDevelopmentType == null &&
        vm.selectedLocationCoordinates == null;
  }

  void _populateFormFromDoc(
      String docId, Map<String, dynamic> data, AgricultureViewModel vm) {
    final agriData = (data['agriculture_infrastructure'] as Map?) ?? {};

    setState(() {
      _locationNameCtrl.text = (agriData['location_name'] ?? '').toString();
      _beneficiariesCtrl.text =
          (agriData['beneficiaries_count'] ?? '').toString();
    });

    vm.setEditingDocId(docId);
    vm.updateDevelopmentCategory(
        (agriData['development_category'] ?? '').toString().isNotEmpty
            ? agriData['development_category']
            : null);
    vm.updateDevelopmentType(
        (agriData['development_type'] ?? '').toString().isNotEmpty
            ? agriData['development_type']
            : null);
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(viewModel.editingDocId != null
              ...
              : 'කෘෂිකාර්මික තොරතුරු සාර්ථකව සුරකින ලදී.'),
              ...
              child: Column(
                ...
                children: [
                  ...
                  _buildDropdownField(
                    ...
                    onChanged: (val) =>
                        viewModel.updateDevelopmentCategory(val),
                  ),
      ...
      children: [
        Row(
          ...
          children: [
            const Row(
              children: [
                Icon(Icons.eco_outlined, color: Color(0xFF16A34A), size: 20),
                SizedBox(width: 8),
                Text(
        ...
        StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
          stream: viewModel.gnAgricultureStream,
          builder: (context, snapshot) {
            ...
            if (docs.isEmpty) {
              return Container(
                ...
                child: const Center(
                  child: Text(
                    'මෙම වසම සඳහා තවමත් කිසිදු කෘෂිකාර්මික දත්තයක් ඇතුළත් කර නොමැත.',
            ...
            return Column(
              children: docs.map((doc) {
                ...
                final agri =
                    (data['agriculture_infrastructure'] as Map?) ?? {};

                final category =
                    (agri['development_category'] ?? '').toString();
                final devType = (agri['development_type'] ?? '').toString();
                final locName = (agri['location_name'] ?? '').toString();
                final coordinates =
                    (agri['map_coordinates'] ?? '').toString();
                final beneficiaries =
                    (agri['beneficiaries_count'] ?? '').toString();
                      ...
                      children: [
                        ...
                        Padding(
                          ...
                          child: Column(
                            ...
                            children: [
                              Text(
                                'වර්ගය: ${devType.isNotEmpty ? devType : "-"} • ප්‍රතිලාභීන්: ${beneficiaries.isNotEmpty ? beneficiaries : "-"}',
                                style: TextStyle(
                              ...
                              onPressed: () async {
                                final confirm = await showDialog<bool>(
                                  ...
                                  builder: (ctx) => AlertDialog(
                                    ...
                                    content: const Text(
                                        'මෙම කෘෂිකාර්මික වාර්තාව ස්ථිරවම මකා දැමීමට ඔබට අවශ්‍යද?'),
                                    actions: [
                                      TextButton(
                                          onPressed: () =>
  ...
  Widget _buildCollectorLocationCard(AgricultureViewModel viewModel) {
