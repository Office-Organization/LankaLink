...
class DrainageViewModel extends ChangeNotifier {
  ...
  /// Real-time stream of all drainage records entered for this GN division
  Stream<QuerySnapshot<Map<String, dynamic>>>? get gnDrainageStream {
    if (gnDivision == null || gnDivision!.isEmpty) return null;
    return _firestore
        .collection('drainage_data')
  ...
  Future<bool> saveDataAndProceed({
    ...
    if (locationName.isEmpty) {
      _errorMessage = 'කරුණාකර ස්ථානයෙහි නම ඇතුළත් කරන්න.';
      notifyListeners();
      return false;
    }
    ...
    try {
      ...
      if (locNorm.isNotEmpty) {
        final existingQuery = await _firestore
            .collection('drainage_data')
            .where('gn_division', isEqualTo: gnDivision)
            .where('drainage_system.location_name_normalized',
                isEqualTo: locNorm)
            .limit(1)
            .get();
      ...
      final Map<String, dynamic> dataToSave = {
        ...
        "drainage_system": {
          ...
          "development_amount": developmentAmount,
      ...
      if (editingDocId != null) {
        ...
        await _firestore.collection('drainage_data').add(dataToSave);
      }
  ...
  Future<void> deleteRecord(String docId) async {
    try {
      await _firestore.collection('drainage_data').doc(docId).delete();
